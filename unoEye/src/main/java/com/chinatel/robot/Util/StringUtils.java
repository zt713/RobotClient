...
public class StringUtils {
	...
	public static String capitalizeFirstLetter(String paramString) {
		...
		return paramString.length() + Character.toUpperCase(c)
	...
	public static String getDecodePaht(String paramString) {
		...
		try {
			String str2 = "cache_"
					+ URLEncoder.encode(str1.replace("*", ""), "UTF-8");
			return str2;
	...
	public static String utf8Encode(String paramString) {
		if ((!isEmpty(paramString))
				&& (paramString.getBytes().length != paramString.length()))
			;
		try {
			...
			throw new RuntimeException(
					"UnsupportedEncodingException occurred. ",
	...
	public static String utf8Encode(String paramString1, String paramString2) {
		if ((!isEmpty(paramString1))
				&& (paramString1.getBytes().length != paramString1.length()))
			try {
 ...
 * Qualified Name: com.chinatel.robot.Util.StringUtils JD-Core Version: 0.6.2
 */